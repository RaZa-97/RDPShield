...
- **HTTPS/TLS — deferred as a FUTURE ENHANCEMENT for the dissertation.** The deployment stays on plain HTTP with port 5000 restricted to the admin IP (accepted for this controlled, single-operator academic project). The plumbing is in place but **left off**: two paths when enabled later — (A) reverse proxy (Caddy + free DuckDNS hostname → trusted Let's Encrypt cert) with `DASHBOARD_BEHIND_PROXY=True` (adds `ProxyFix` so scheme + real client IP in the audit log are correct); (B) direct cert via `DASHBOARD_SSL_CERT`/`DASHBOARD_SSL_KEY` → `app.run(ssl_context=...)`. `DASHBOARD_USE_HTTPS=True` then marks the cookie Secure. All flags are `getattr`-optional (default off) and documented in `config.example.py`; full recipe in `INSTALL.md` §11 (now a collapsed "future enhancement" section), phone-install steps in §12.
...
- **Detection:** brute force (5/60s), slow-and-low (10/600s), password spray (4 users/300s), **persistent/low-and-slow (15 failures/24h any pacing)**, and **reputation/threat-intel** — for low-volume IPs that trip no count rule, checks AbuseIPDB (cached, quota-safe) + VirusTotal (for flagged IPs) past a 3-attempt floor; tiered response: SMS the SOC at AbuseIPDB ≥50%, auto-block at ≥85% (or ≥50% + VT malicious). New config `REPUTATION_*`; `reputation_alert` added to `SMS_ALERT_TYPES`; new `abuse_cache` table.
- **Campaign / coordinated-attack detector** (`campaign_detector.py`, run on a timer by the agent every `CAMPAIGN_CHECK_INTERVAL_HOURS`): rolling 7-day correlation surfacing campaigns no single-window detector catches — a determined IP active across many days, a country attacking with many rotating IPs, and attacks recurring in the same time-of-day band (the "scheduled" flag). Tiered: SMS the SOC for every campaign + a dashboard "Campaigns (7-day)" tracker; auto-block only the worst single-IP campaigns (countries alert-only). New config `CAMPAIGN_*`; `campaign_alert` in `SMS_ALERT_TYPES`; new `campaigns` table; `/api/campaigns` route.
